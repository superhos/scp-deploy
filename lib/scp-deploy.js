...
class scpDeploy {
  ...
  main(){
    ...
    if (this.config.enableFilter){
      for(var i in this.config.src){
        ...
        this.execCopy( this.config.src[i] )
      }
  ...
  execCopy(opts){
    fs.copySync(opts.src, utils.pathResolve(defaultConfig.tempPath), {
      filter: (src, dest)=>{
        var isExclude = this.isExclude(src)
        return !isExclude
      }
    })
  }
    ...
    this.config.exclude.map(item=>{
      ...
      if ( REG_ALL_FOLDER.test(item.src) ){
        ...
        if ( src.indexOf(filter)>-1 ){
          isExclude = true
          // console.log('exclude folder', item.src)
        }
      }
      // exclude filetype
      else if (item.src == '/'){
