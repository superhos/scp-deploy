...
class scpDeploy {
  ...
  checkSourcePath(src){
    if (_.isArray(src)) return true;
    src = src.replace(/\**[^*]*$/g,'')
    if (fs.statSync(src).isFile()) return true
    var files = fs.readdirSync(src)
    return files.length!=0
  }

  checkConfig(config){
    if (!config.username) return 'config.username must be required'
    if (!config.password) return 'config.password must be required'
    if (!config.port) return 'config.port must be required'
    if (!config.src) return 'config.src must be required'
    if (!config.path) return 'config.path must be required'
    if (!config.host) return 'config.host must be required'
    return true
  }
  ...
  copy(options) {
    ...
    fs.copySync(options.src, './tmp/', {
      filter: (src, dest) => {
        ...
        if (_.isArray(this.options.exclude)) {
          let isExclude = false;
          this.options.exclude.forEach(ex=>{
            if ( src.indexOf(ex)>-1 ) {
              isExclude = true
              return false;
            }
          ...
          if (isExclude){
            console.log(`exclude: ${path.resolve(src)}`.yellow)
            return false;
          }
        }
        // filter subpath folder when don`t contain **
        if (!options.includeSub&&s.isDirectory() && src!=options.src) {
          return false;
        }
